...
public class DisplayManager {
	...
	public static void displayPage(PlayerSession session, int page) {
		...
		for (int i = (page-1) * maxLines; i < ((page-1) * maxLines) + maxLines; i++) {
			...
			sendLine(session.getSender(), "&7" + entry.getDataid() + " &c" + entry.getDate().substring(5) + " &7" + entry.getPlayer() + " &c" + action + " &7" + entry.getWorld() + ":" + entry.getX() + "," + entry.getY() + "," + entry.getZ());
			sendLine(session.getSender(), "   &7Data: &c" + data);
		}
