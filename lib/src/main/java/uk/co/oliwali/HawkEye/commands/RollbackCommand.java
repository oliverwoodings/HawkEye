...
public class RollbackCommand extends BaseCommand {
   ...
   public RollbackCommand() {
      ...
      this.usage = "<parameters> <- rollback changes";
   }
   ...
   public boolean execute() {
      if(this.session.doingRollback()) {
         ...
         new SearchQuery(new RollbackCallback(this.session, Rollback.RollbackType.GLOBAL), parser, SearchQuery.SearchDir.DESC);
         return true;
      }
   }
